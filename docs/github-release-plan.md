...
2. Pushing a matching version tag, such as `v0.8.4`, starts `.github/workflows/release.yml`.
...
Create `.github/workflows/release.yml`, triggered only by a protected semantic tag such as `v0.8.4` and an optional manual dispatch for release candidates.
...
1. Build `v0.8.4` as a draft release and test downloads on clean Windows, macOS Apple Silicon, macOS Intel, and Linux machines before publication.
