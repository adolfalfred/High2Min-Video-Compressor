...
2. Pushing a matching version tag, such as `v0.10.0`, starts `.github/workflows/release.yml`.
...
Create `.github/workflows/release.yml`, triggered only by a protected semantic tag such as `v0.10.0` and an optional manual dispatch for release candidates.
...
1. Build `v0.10.0` as a draft release and test downloads on clean Windows, macOS Apple Silicon, macOS Intel, and Linux machines before publication.
...
- Keep binary self-replacement deferred until code signing is available. Version 0.10.0 will download and verify the matching archive in-app but will not silently execute or replace the running application.
- Defer Windows ARM64, Linux ARM64, Homebrew, WinGet, and package installers until the four core artifacts have a stable, signed release process.
