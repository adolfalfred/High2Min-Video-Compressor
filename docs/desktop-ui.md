...
**Analyze ADT changes** first shows the inferred source filename → current ADT page → runtime mapping → normalized `page_N.mp4` destination and the exact number of mutations, removals, warnings, and blockers without writing. Page zero is supported for a front cover. If joined pages changed a Git-tracked spine, High2Min resolves old source numbers through stable historical page hrefs and warns when a former section now belongs to a combined page. A JSON/CSV mapping can alternatively target an exact current href or section ID. Merge mode preserves existing mappings and unreferenced legacy source-segment videos; replace mode lists removals and asks for confirmation.
