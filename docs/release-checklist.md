...
```sh
git tag -a v0.11.9 -m "High2Min Video Compressor 0.11.9"
...
- [ ] Confirm `High2Min Video Compressor.exe` opens without a terminal and `high2min.exe --version` prints `0.11.9` in PowerShell.
- [ ] Confirm the automatic startup check and manual update action report that published `0.11.9` is current.
...
- [ ] Confirm merge preserves untouched in-book legacy paths such as `../../../../Videos_web/page_1.mp4`, rejects paths that escape the book, and migrates only selected replacements to the canonical video directory.
- [ ] Confirm merge accepts cache-busted mappings such as `page_1.mp4?v=17`, checks the physical `page_1.mp4`, and advances the replaced mapping's cache version.
...
- [ ] Confirm legacy `CURRENT_CONFIG` and `CURRENT_VIDEOS` preloader fallbacks match the refreshed embedded resources after publishing.
...
If a problem is found, delete or retain the faulty draft for diagnosis, correct the source, increment the project version, and create a new patch tag. Do not reuse a release tag or silently replace a published asset.
